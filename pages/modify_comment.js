...
export default function Modify(props){
    ...
    useEffect(() => {
        async function getComment(){
            const response = await axios.get("http://127.0.0.1:8000/board/getComment/", {params : {'id': commentId}});
    ...
    async function modify(){
        if(content){
            ...
            const response = await axios.post("http://127.0.0.1:8000/board/modifyComment/", form);
            router.back();
        }
        else{
            alert("내용이 없습니다.");
